...
int*** DumpyNode::combines = readCombines();
...
void materialize1stLayer(string datafn, DumpyNode* root, int *navids, string index_dir, unsigned short*sax_tbl){
    ...
    unordered_set<DumpyNode*>().swap(visited);
...
void materializeInterNode(DumpyNode *node, unsigned short *saxes) {
    ...
    unordered_map<DumpyNode*, SAX_BUF_UNIT>sax_buffer;
    for(int i=0;i<node->size;++i){
        int offset = node->offsets[i];
        unsigned short *sax = saxes + offset * Const::segmentNum;
        DumpyNode*target = node->route(sax);
    ...
    for(auto &iter: sax_buffer){
        string outfile = Const::idxfn + iter.first->getFileName() + "_sax";
    ...
    unordered_map<DumpyNode*, SAX_BUF_UNIT>().swap(sax_buffer);
...
//            assert(nodeIn1stLayer[i].size > Const::th);
    ...
    thread IO(materialize1stLayer, datafn, root, navids, Const::idxfn, saxes);
    ...
    for(int i=0;i<Const::vertexNum;++i){
        ...
        if(++j%5000 == 0)  Const::logPrint(to_string(j) + " nodes in the 1st layer has been processed.");
    }
